...
export const generateRefreshToken = (user: User, ip: string): string => {
  ...
  if (!expiry) {
    throw new Error("JWT Refresh Expiry is not defined");
  }
  ...
  return jwt.sign({ id: user.id, ip }, secret, options);
};
...
export const verifyRefreshToken = (token: string, ip: string): any => {
  ...
  if (decoded.ip !== ip) {
    throw new Error("Refresh token invalid: IP mismatch");
  }
  return decoded;
};
